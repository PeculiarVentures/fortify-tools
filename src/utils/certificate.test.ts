import { ICertificateProps, ICertificateSubjectProps } from '../types';
...
describe('Certificate Utilities', () => {
  describe('certificateSubjectToString', () => {
    const dafaultAttrs = {
      ...
      SN: ['Surname'],
      E: ['email@example.com'],
    } as unknown as ICertificateSubjectProps;
    ...
    it('Should return an empty string if no attributes are provided', () => {
      expect(
        certificateSubjectToString({} as unknown as ICertificateSubjectProps),
      ).toEqual('');
    });
  });
