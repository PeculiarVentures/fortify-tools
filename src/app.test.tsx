...
describe('<App />', () => {
  ...
  const certificateRequestsMock = [
    {
      id: '1',
      providerID: 'provider1',
      subject: { CN: ['Certificate request test 1'] },
      raw: new ArrayBuffer(1),
      type: 'request',
    },
  ];
  ...
  it('Should open create certificate (CSR) dialog', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('heading', { name: /Create Certificate Signing Request \(CSR\)/ }),
      ).toBeInTheDocument();
    });
  });
